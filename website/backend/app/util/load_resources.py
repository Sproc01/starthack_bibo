...
def fetch_historical_weather_data(start_date: str, end_date: str):
                ...
                "codes": [
                    {
                        "code": 11,
                        "level": "2 m above gnd",
                        "aggregation": "mean"  # Average Temperature
                    }
                ]
            },
            {
                ...
                "codes": [
                    {
                        "code": 11,
                        "level": "2 m above gnd",
                        "aggregation": "max"  # Max Temperature
                    }
                ]
            },
            {
                ...
                "codes": [
                    {
                        "code": 11,
                        "level": "2 m above gnd",
                        "aggregation": "min"  # Min Temperature
                    }
                ]
            },
            {
                ...
                "codes": [
                    {
                        "code": 144,
                        "level": "0-7 cm down",
                        "aggregation": "mean"  # Average soil moisture
                    }
                ]
            },
            {
                ...
                "codes": [
                    {
                        "code": 260,
                        "level": "2 m above gnd",
                        "aggregation": "sum"  # Evaporation sum
                    }
                ]
            }
        ]
    ...
    dates = [datetime.strptime(date.split('T')[0], '%Y%m%d').strftime('%Y-%m-%d') 
...
class GlobalResources(metaclass=SingletonMeta):
    ...
    def _update_database(self):
        try:
            ...
            start_date = (datetime.strptime(latest_date, c.METEOBLUE_DATE_FORMAT) + timedelta(days=1)).strftime(c.METEOBLUE_DATE_FORMAT)
            end_date = (datetime.now() - timedelta(days=1)).strftime(c.METEOBLUE_DATE_FORMAT)

            # Database is already up-to-date
            if datetime.strptime(start_date, c.METEOBLUE_DATE_FORMAT) > datetime.strptime(end_date, c.METEOBLUE_DATE_FORMAT):
                weather_db.close()
                return
